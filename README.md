...
**4배 빨라지고 편차도 5.7배 → 1.4배로 줄어듭니다.** 분류·추출·요약처럼 생각할 게 많지 않은 작업이면 `'none'`이 맞습니다. 반대로 코드를 짜거나 다단계 추론이 필요하면 올려야 합니다 — 빨라지는 대신 품질을 내주는 거래입니다.

값은 `'none' | 'low' | 'medium' | 'high' | 'xhigh' | 'max'`입니다.
