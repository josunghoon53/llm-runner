...
`openai-subscription`을 쓰기로 했다면 **작업이 길 때만 `reasoningEffort`를 내려라.** 지정하지 않으면 Codex 기본값(medium)으로 돈다.

판단 기준은 "생각할 게 많은가"가 아니라 **"실제로 오래 걸리는가"**다. 다이얼은 생각하는 시간만 줄이므로, 원래 짧은 작업은 줄일 것이 없다.

| 작업 | 권장 | 실측 근거 (2026-09-24, 총 442회) |
|---|---|---|
| 웹 검색·도구를 쓰는 조사 (60초 이상) | **`'low'`** | 시간 30~48% 감소, 품질 유지 |
| 짧은 분류·추출 (10초 이하) | 손대지 마라 | 시간 1~9%만 감소 — 얻을 게 없다 |
| 짧은 논리·계산 | **기본값 유지** | `'none'`은 다단계 계산 12회 중 **0회 정답** |

- `'none'`은 다단계 추론이 필요 없는 작업에만 써라. 계산·날짜 순서·여러 조건 조합이 섞이면 무너진다.
- 코드 생성이나 깊은 디버깅에서만 올려라 — 품질과의 거래다.
